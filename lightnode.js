...
/*
 * 
 * 23 Feb 2011 (Craig S. Greenhouse)
 * 
 * Code cleaned up to be more in line with accepted coding standards :
 * 
 * Braces added around single line statements after "if"s
 * 
 * Semicolons added to line ends
 * 
 * Variable and function declarations moved towards the top (i.e. before they are used)
 * 
 * Comments  added to the end of closing brackets after long code sections to 
 * show which bit of code they are closing 
 * 
 * Code run through jslint to check for syntax errors
 * 
 */
...
It's a novel way of conceptualising the nature and creation of constructors and prototypes,
...
for (prop in parent) {
  if (parent[prop] instanceof Function) {
...
var Server = type(EventEmitter, function() {
  ...
  this.receiveRequest = function(req, resp) {
    ...
    if (delegateTo == this){ this.emitRequest(req, resp); }
    else {
      if (delegateTo instanceof Server) { delegateTo.receiveRequest(req, resp); }
      else if (typeof delegateTo == 'function') { delegateTo(req, resp); }
      else { logError("Couldn't delegate the request to the specified object."); }
    }
  }; // this.receiveRequest
...
exports.FileServer = type(exports.HttpServer, function() {
  ...
  this.serveRequest = function(req, resp) {
    ...
    var getFile = function(path) {
      if (!fileCache[path]) { fileCache[path] = new exports.File(path); }
  ...
  this.sendFile = function(req, resp, file) { 
    var self = this;
    if (!file) { return this.sendNone(req, resp); }
    ...
    file.stat(function(statErr) {
      ...
      if (statErr) { return self.sendNone(req, resp); }
      ...
      if (ext in mimeTypes) { headers['content-type'] = mimeTypes[ext]; }
...
exports.File = function(filename) {
  ...
  this.stat = function(F) {
    ...
    if (statLastCalled && statLastCalled > Date.now() - 500) {
      if (!statResult) { statWaitors.push(F); }
      else { F(statResult[0], statResult[1]); }
    }
    else {
      statWaitors.push(F);
  ...
  this.readFile = function(F) {
    ...
    if (hasCalledReadFile) {
      if (!readFileResult) { readFileWaitors.push(F); }
      else { F(readFileResult[0], readFileResult[1]); }
    } else {
